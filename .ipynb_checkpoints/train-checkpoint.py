...
for epoch in range(start_epoch, cfg["epochs"]):
    ...
    for x_l, y_l in tqdm(labeled_loader, desc=f"[{method}] Epoch {epoch}"):
        ...
        if unlabeled_iter is not None and loss_type != "sup_only":
            ...
            with torch.no_grad():
                logits_u, z_u = model(x_u)
                probs_u = torch.softmax(logits_u, dim=1)
                conf_u, pseudo_u = probs_u.max(dim=1)

            threshold = cfg.get("pseudo_threshold", 0.7)
            mask = conf_u >= threshold

            if mask.sum() > 0:
                z_all = torch.cat([z_l, z_u[mask]], dim=0)
                all_labels = torch.cat([y_l, pseudo_u[mask]], dim=0)
            else:
                z_all = z_l
                all_labels = y_l
        else:
